...
Form::Form()
	: name("default name"), target("default target"), grade_to_sign(150),
...
Form::Form(const std::string &name, int grade_to_sign,
	...
	: name(name), target(target), grade_to_sign(grade_to_sign),
		grade_to_execute(grade_to_execute), isSigned(false) {

}

Form::Form(const Form &another)
	: name(another.name), target(another.getTarget()),
...
void Form::execute(Bureaucrat const & executor) const{
	isGradeRight(executor.getGrade(), getGradeToExecute());
}
